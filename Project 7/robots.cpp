...
bool Robot::getAttacked(int dir)  // return true if dies
{
	...
	if (m_health == 0)
		return true;
	else if (m_arena->determineNewPosition(m_row, m_col, dir) == true)
		return false;
	else
		return true;
...
int Arena::nRobotsAt(int r, int c) const
{
	...
	for (int i = 0; i < m_nRobots; i++)
	{
		const Robot* YY = m_robots[i];
		if (YY->row() == r && YY->col() == c)
		{
			count++;
		}
	}
...
void Arena::display() const
{
	...
	for (int t = 0; t < r0w; t++)
	{
		for (int h = 0; h < c0l; h++)
		{
			int numR0b0ts = nRobotsAt(t + 1, h + 1); 

			if (numR0b0ts == 1)
			{
				grid[t][h] = 'R';
			}
			else if (numR0b0ts >= 2 && numR0b0ts <= 8)
			{
				grid[t][h] = numR0b0ts + '0';
			}
			else if (numR0b0ts >= 9)
			{
				grid[t][h] = '9';
...
{
	// Don't add a robot if the maximum robot capacity is reached
...
bool Arena::attackRobotAt(int r, int c, int dir)
{
	for (int i = 0; i < m_nRobots; i++)
	{
		...
		{
			// if the robot will die after this attack
			if (m_robots[i]->getAttacked(dir) == true)
			{
				delete m_robots[i];
				m_robots[i] = nullptr;

				// loops from the current robot to all robots after it
				// pointer at old location now points to the next robot, and so on
				for (int k = i; k < m_nRobots - 1; k++)
				{
					m_robots[k] = m_robots[k + 1];
				}
				// after deleting a robot, reduces total number of robots by 1
				m_nRobots--;
				return true;
			}
		}
...
bool Arena::moveRobots()
{
	for (int k = 0; k < m_nRobots; k++)
	{
		Robot* rr = m_robots[k];
		rr->move();
		if (rr->row() == m_player->row() && rr->col() == m_player->col())
			m_player->setDead();
	}
